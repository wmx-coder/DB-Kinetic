...
# ===================== 仅读取你的预测文件 =====================
df = pd.read_pickle("/mnt/usb3/wmx/kcat_km/case/predict.pkl")
...
for combo_name, group in df.groupby("enz_sub_combo"):
    ...
    # 计算 SCC PCC
    y_true = group["log10_kcat_over_Km"].values
    y_pred = group["pred_fusion_log10[kcat/Km]"].values
    if len(y_true) >= 2:
...
keep_cols = [
    "UniProtID", "EC", "Smiles",
    "log10_kcat_over_Km", "pred_fusion_log10[kcat/Km]",
    "EnzymeType", "Substrate"
]
df_out = df_selected[keep_cols].copy()

# ===================== 保存 =====================
save_path = "/mnt/usb3/wmx/kcat_km/case/top16_selected_samples.csv"
