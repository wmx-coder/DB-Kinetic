...
# ===================== 仅读取你的预测文件 =====================
df = pd.read_pickle("/mnt/usb3/wmx/km/case/predict.pkl")
...
for combo_name, group in df.groupby("enz_sub_combo"):
    ...
    # 计算 SCC PCC
    y_true = group["log10_Km"].values
    y_pred = group["pred_log10[Km(mM)]"].values
    if len(y_true) >= 2:
...
keep_cols = [
    "UniProtID", "EC", "Smiles",
    "log10_Km", "pred_log10[Km(mM)]",
    "EnzymeType", "Substrate"
]
df_out = df_selected[keep_cols].copy()

# ===================== 保存 =====================
save_path = "/mnt/usb3/wmx/km/case/top16_selected_samples.csv"
