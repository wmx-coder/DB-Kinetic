...
if __name__ == "__main__":
    ...
    parser.add_argument("-model_dpath", type=str, default="/mnt/usb1/wmx/catapro/recover/km",  # 修改模型路径
                        help="Input. The path of saved models.")
    parser.add_argument("-device", type=str, default="cuda",
                        help="Input. The device: cuda or cpu.")
    parser.add_argument("-out_fpath", type=str, default="./km20.pkl",  # 修改输出文件名为km20.pkl
    ...
    # 移除Km_models子目录拼接，直接使用指定路径
    km_model_dpath = model_dpath  # 模型文件直接在指定目录下
    device = args.device
    ...
    for fold in unique_folds:
        try:
            ...
            model_file_path = f"{km_model_dpath}/{fold}_bestmodel.pth"
            km_model.load_state_dict(th.load(model_file_path, map_location=device))
            model_cache[fold] = km_model
            print(f"成功加载fold {fold} 的KmModel（路径：{model_file_path}）")
        except Exception as e:
            raise ValueError(f"加载fold {fold} 的Km模型失败：{e}，模型路径：{model_file_path}")
    ...
    csv_out_path = args.out_fpath.replace(".pkl", ".csv")  # 输出为km20.csv
