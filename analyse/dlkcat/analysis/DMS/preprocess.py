...
#
# def main() :
#     with open('/mnt/usb1/wmx/dlkcat/dlkcat_duibi/analysis/data/DERA_clean.json', 'r') as infile :
...
#     output_dir = '/mnt/usb1/wmx/dlkcat/dlkcat_duibi/analysis/data/dera/input/'
...
#     dump_dictionary(fingerprint_dict, '/mnt/usb1/wmx/dlkcat/dlkcat_duibi/analysis/data/dera/input/fingerprint_dict.pickle')
#     dump_dictionary(atom_dict, '/mnt/usb1/wmx/dlkcat/dlkcat_duibi/analysis/data/dera/input/atom_dict.pickle')
#     dump_dictionary(bond_dict, '/mnt/usb1/wmx/dlkcat/dlkcat_duibi/analysis/data/dera/input/bond_dict.pickle')
#     dump_dictionary(edge_dict, '/mnt/usb1/wmx/dlkcat/dlkcat_duibi/analysis/data/dera/input/edge_dict.pickle')
#     dump_dictionary(word_dict, '/mnt/usb1/wmx/dlkcat/dlkcat_duibi/analysis/data/dera/input/sequence_dict.pickle')
#
#
...
#     global word_dict, atom_dict, bond_dict, fingerprint_dict, edge_dict
#     global proteins, compounds, adjacencies, regression
#
...
#         adjacencies.append(adjacency)
#         proteins.append(words)
#         regression.append(np.array([0.0]))  # 无标签，占位
#
#     # 创建独立输入文件夹
#     name = os.path.basename(json_path).replace(".json", "")
#     output_dir = os.path.join(base_output_dir, name, "input")
#     os.makedirs(output_dir, exist_ok=True)
#
#     # 保存文件
#     np.save(os.path.join(output_dir, "compounds.npy"), np.array(compounds, dtype=object), allow_pickle=True)
#     np.save(os.path.join(output_dir, "adjacencies.npy"), np.array(adjacencies, dtype=object), allow_pickle=True)
#     np.save(os.path.join(output_dir, "proteins.npy"), np.array(proteins, dtype=object), allow_pickle=True)
#     np.save(os.path.join(output_dir, "regression.npy"), np.array(regression, dtype=np.float32))
#
#     dump_dictionary(fingerprint_dict, os.path.join(output_dir, "fingerprint_dict.pickle"))
#     dump_dictionary(atom_dict, os.path.join(output_dir, "atom_dict.pickle"))
#     dump_dictionary(bond_dict, os.path.join(output_dir, "bond_dict.pickle"))
#     dump_dictionary(edge_dict, os.path.join(output_dir, "edge_dict.pickle"))
#     dump_dictionary(word_dict, os.path.join(output_dir, "sequence_dict.pickle"))
#
...
# # ===================== 批量处理 12 个文件 =====================
# if __name__ == '__main__':
#     BASE_DIR = "/mnt/usb1/wmx/dlkcat/dlkcat_duibi/analysis/DMS"
#
#     JSON_FILES = [
#         f"{BASE_DIR}/EcTL_sbt1.json",
#         f"{BASE_DIR}/EcTL_sbt2.json",
#         f"{BASE_DIR}/HIS3_sbt1.json",
#         f"{BASE_DIR}/HIS3_sbt2.json",
#         f"{BASE_DIR}/HIS7_sbt1.json",
#         f"{BASE_DIR}/HIS7_sbt2.json",
#         f"{BASE_DIR}/si-dbs_ub_sbt1.json",
#         f"{BASE_DIR}/si-dbs_ub_sbt2.json",
#         f"{BASE_DIR}/Ssdata_ub_sbt1.json",
#         f"{BASE_DIR}/Ssdata_ub_sbt2.json",
#         f"{BASE_DIR}/Ttdata_sbt1.json",
#         f"{BASE_DIR}/Ttdata_sbt2.json"
#     ]
#
#     for fp in JSON_FILES:
#         if os.path.exists(fp):
#             process_single_json(fp, BASE_DIR)
#         else:
#             print(f"⚠️ 文件不存在：{fp}")
#
#     print("🎉🎉🎉 全部 12 个文件预处理完成！")
...
def extract_fingerprints(atoms, i_jbond_dict, radius):
    ...
    nodes, edges = atoms, i_jbond_dict
    for _ in range(radius):
        fps = []
        for i, je in edges.items():
            nb = [(nodes[j], e) for j,e in je]
            fps.append(fingerprint_dict[(nodes[i], tuple(sorted(nb)))])
        nodes = fps
        new_edges = defaultdict(lambda: [])
        for i, je in edges.items():
            for j,e in je:
                key = tuple(sorted((nodes[i], nodes[j])))
                new_edges[i].append((j, edge_dict[(key, e)]))
        edges = new_edges
...
# ===================== 单底物处理 =====================
def process_single_json(json_path, out_base):
    reset_global_dicts()
    ...
    for entry in data:
        ...
        atoms = create_atoms(mol)
        edge_dict = create_ijbonddict(mol)
        fps = extract_fingerprints(atoms, edge_dict, radius)
        adj = create_adjacency(mol)
        words = split_sequence(seq, ngram)

        compounds.append(fps)
        adjacencies.append(adj)
        proteins.append(words)
        regression.append([0.0])

    # 输出到同名文件夹 input
    name = os.path.splitext(os.path.basename(json_path))[0]
    out_dir = os.path.join(out_base, name, "input")
    os.makedirs(out_dir, exist_ok=True)

    np.save(out_dir+"/compounds.npy", np.array(compounds, dtype=object), allow_pickle=True)
    np.save(out_dir+"/adjacencies.npy", np.array(adjacencies, dtype=object), allow_pickle=True)
    np.save(out_dir+"/proteins.npy", np.array(proteins, dtype=object), allow_pickle=True)
    np.save(out_dir+"/regression.npy", np.array(regression, dtype=np.float32))

    dump_dict(fingerprint_dict, out_dir+"/fingerprint_dict.pickle")
    dump_dict(atom_dict, out_dir+"/atom_dict.pickle")
    dump_dict(bond_dict, out_dir+"/bond_dict.pickle")
    dump_dict(edge_dict, out_dir+"/edge_dict.pickle")
    dump_dict(word_dict, out_dir+"/sequence_dict.pickle")

    print(f"✅ 完成：{name:15s} | 样本数：{len(data)}")


# ===================== 只运行这 3 个！=====================
if __name__ == "__main__":
    BASE = "/mnt/usb1/wmx/dlkcat/dlkcat_duibi/analysis/DMS"

    TARGET_FILES = [
        f"{BASE}/HIS3.json",
        f"{BASE}/HIS7.json",
        f"{BASE}/si-dbs_ub.json",
    ]

    for fp in TARGET_FILES:
        if os.path.exists(fp):
            process_single_json(fp, BASE)
        else:
            print(f"❌ 不存在：{fp}")
