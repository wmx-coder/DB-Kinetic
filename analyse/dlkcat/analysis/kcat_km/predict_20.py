...
# ====================== 配置项（kcat-km专用，已替换所有路径） ======================
# 1. kcat-km小数据集路径（PKL）
SMALL_DATA_PATH = "/mnt/usb3/wmx/analyse/kcat-km_data_filtered_N20.pkl"
# 2. kcat-km全量数据集路径（JSON）
FULL_DATA_PATH = "/mnt/usb1/wmx/dlkcat/Data/database/Kcat_km_combination_0918.json"
# 3. kcat-km全量预测结果路径（TSV）
PREDICTION_PATH = "/mnt/usb1/wmx/dlkcat/dlkcat_duibi/analysis/kcat_km/kcat_km_prediction.tsv"
# 4. 输出匹配结果路径
OUTPUT_PATH = "./kcat_km_N20_1.pkl"
...
def main():
    ...
    print(f"✅ kcat-km小数据集加载成功：{len(df_small)} 条样本")
    ...
    print(f"✅ kcat-km全量数据集加载成功：{len(df_full)} 条样本")

    # 加载kcat-km全量预测结果（TSV）
    df_pred = pd.read_csv(PREDICTION_PATH, sep='\t')
    print(f"✅ kcat-km全量预测结果加载成功：{len(df_pred)} 条样本")
    ...
    # ---------------------- 3. 匹配：kcat-km小数据集 ↔ 全量数据集 ----------------------
    print("\n" + "=" * 60)
    print("3. 开始精准匹配...")
    # 步骤1：小数据集关联全量数据集的Sample_Index（溯源全量索引）
    df_match = pd.merge(
        df_small,  # 小数据集（左表）
        df_full[['Unique_Key', 'Sample_Index', 'Value']],  # 全量数据集仅取匹配字段（Value=kcat/km真实值）
    ...
    # 适配kcat-km预测结果的字段名（Pred_kcat_km_log2/Pred_kcat_km_original）
    # 先检查预测结果字段名，兼容两种命名
    pred_log2_col = 'Pred_kcat_km_log2' if 'Pred_kcat_km_log2' in df_pred.columns else 'Pred_Kcat_log2'
    pred_original_col = 'Pred_kcat_km_original' if 'Pred_kcat_km_original' in df_pred.columns else 'Pred_Kcat_original'
    ...
    print(f"📊 匹配统计：")
    print(f"   kcat-km小数据集总样本：{total_small} 条")
    ...
    if match_fail > 0:
        ...
        # 保存失败样本
        df_fail.to_csv("./kcat_km_match_failed_samples.csv", index=False, encoding='utf-8')
        print(f"⚠️  匹配失败样本已保存至：./kcat_km_match_failed_samples.csv")
        print(f"   失败样本预览（前3条）：")
    ...
    # 重命名预测字段（统一为kcat-km语义，更直观）
    rename_map = {
        pred_log2_col: 'Pred_kcat_km_log2',  # kcat/km预测值（log2）
        pred_original_col: 'Pred_kcat_km_original',  # kcat/km预测值（原始）
        'Value': 'True_kcat_km_original',  # kcat/km真实值（全量JSON）
        'Model_Path': 'Used_Model_Path',  # 使用的模型路径
    ...
    core_cols = [
        'Smiles', 'Sequence', 'kcat(s^-1)', 'Km(M)', 'fold',
        'True_kcat_km_original', 'Pred_kcat_km_original', 'Used_Model_Path'
    ]
    # 过滤存在的列（避免KeyError）
    ...
    # 数值保留4位小数
    numeric_cols = ['kcat(s^-1)', 'Km(M)', 'True_kcat_km_original', 'Pred_kcat_km_original']
...
if __name__ == '__main__':
    main()
    print("\n" + "=" * 60)
    print("🎉 kcat-km数据集匹配完成！所有操作已执行完毕。")