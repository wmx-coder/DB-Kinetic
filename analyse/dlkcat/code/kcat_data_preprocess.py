...
# 1. 定义数据集路径
csv_path = "/mnt/usb1/wmx/catapro/datasets/kcat-data_0.4simi-10fold.csv"
...
smiles_with_dot[["Smiles", "Sequence", "kcat(s^-1)", "fold"]].to_csv(smiles_dot_export_path, index=False)
...
# ---------------------- 分步统计剔除样本数 ----------------------
# 步骤1：剔除kcat≤0的样本，统计该步骤剔除数量
kcat_before = len(df)
df = df[df["kcat(s^-1)"] > 0].reset_index(drop=True)
kcat_after = len(df)
kcat_invalid_count = kcat_before - kcat_after
print(f"\n【步骤1】剔除kcat≤0的样本数：{kcat_invalid_count}")
print(f"步骤1后样本数：{kcat_after}")
...
print(f"其中：")
print(f"  - kcat≤0的样本：{kcat_invalid_count} 个")
...
# ---------------------- 后续逻辑（修复JSON保存） ----------------------
# 1. 重命名kcat列为DLKcat标准名称"Value"
df.rename(
    columns={"kcat(s^-1)": "Value"},
    inplace=True
)
...
json_save_path = "../../Data/database/Kcat_combination_0918.json"
...
folds_save_path = "../../Data/input/folds.npy"
