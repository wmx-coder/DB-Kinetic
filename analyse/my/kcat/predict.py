...
# 导入自定义模块（需确保路径正确）
from model_freeze import Model_Regression, ActivityModel_Freeze as ActivityModel  # 按实际模型版本选择
from util_check import RMSELoss  # 保持与训练脚本一致的依赖
...
def load_model(fold, model_save_dir, kcat_km_model_dir, km_model_dir, alpha, device):
    """
    加载指定fold的模型
    :param fold: 要加载的fold编号（0-9）
    :param model_save_dir: 训练好的kcat模型保存目录
    :param kcat_km_model_dir: kcat/km预训练模型目录
    :param km_model_dir: Km预训练模型目录
    :param alpha: 融合模型的左路权重
    :param device: 运行设备
    :return: 加载好权重的ActivityModel模型
    """
...
def infer_by_fold(data_df, model_save_dir, kcat_km_model_dir, km_model_dir, alpha, batch_size, device):
    """
    按fold分组进行预测
    :param data_df: 包含酶特征、底物特征、fold列的DataFrame
    :param model_save_dir: kcat模型保存目录
    :param kcat_km_model_dir: kcat/km预训练模型目录
    :param km_model_dir: Km预训练模型目录
    :param alpha: 融合模型左路权重
    :param batch_size: 预测批次大小
    :param device: 运行设备
    :return: 预测结果数组（log10(kcat)）
    """
    # 1. 数据预处理与校验
    # 酶特征校验
    ezy_feats_list = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("-inp_fpath", type=str,
                        default="/mnt/usb3/wmx/kcat/case/kcat_final_valid_samples.pkl",
    ...
    parser.add_argument("-out_fpath", type=str,
                        default="/mnt/usb3/wmx/kcat/case/predict.pkl",
                        help="预测结果输出文件路径")
    # 超参数
    ...
    # 1. 加载待预测数据（需确保是pickle或csv格式，若为csv需先加载特征）
    # 注意：若数据是csv且特征以字符串存储，需先转换为np.array
