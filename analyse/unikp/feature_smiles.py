...
# -------------------------- 配置：仅保留数据集处理相关路径 --------------------------
# 原始pkl数据集路径（只读，不修改）
RAW_DATA_PATH = '/mnt/usb3/wmx/kcat/kcat-data_feats_complete.pkl'
...
PROCESSED_DATA_PATH = "PreKcat_new/10fold/data_with_id_log_processed.pkl"
...
def main():
    ...
    # -------------------------- 步骤5：处理log10_kcat列（复用/新增） --------------------------
    print(f"\n===== 开始处理log10_kcat列 =====")
    if 'log10_kcat' in df.columns:
        # 复用已有列，确保列名统一，不修改原始值
        df['log10_kcat'] = df['log10_kcat'].copy()
        print(f"✅ 已复用现有log10_kcat列，值保持不变")
    else:
        # 无现有列，基于kcat(s^-1)计算
        print(f"未检测到log10_kcat列，基于kcat(s^-1)计算...")

        def calc_log10_kcat(x):
            if x <= 0:
                return -10000000000  # 异常值标记（与后续训练逻辑对齐）
            return math.log10(x)

        df['log10_kcat'] = df['kcat(s^-1)'].apply(calc_log10_kcat)
        print(f"✅ 已新增log10_kcat列，异常值标记为：-10000000000")
