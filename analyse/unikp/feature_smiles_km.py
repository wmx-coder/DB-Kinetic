...
# -------------------------- 配置：修改为Km数据集相关路径 --------------------------
# 原始pkl数据集路径（Km数据集，只读，不修改）
RAW_DATA_PATH = '/mnt/usb3/wmx/km/Km-data_ezy_filled.pkl'
# 底物：唯一SMILES→ID映射 + ID→特征（核心存储，路径适配Km任务）
SMILES_ID_FEAT_PATH = "PreKm_new/10fold/km_smiles_id_feat.pkl"
# 处理后（副本删除指定列+新增ID/Log列）的Km数据集保存路径（pkl格式，保留ezy_feat原始格式）
PROCESSED_DATA_PATH = "PreKm_new/10fold/km_data_with_id_log_processed.pkl"
...
def main():
    """主流程：仅负责Km数据集处理（单一职责），完全保留ezy_feat原始格式
    1. 加载原始Km pkl数据（只读）
    2. 创建独立副本（不影响原始数据）
    3. 处理SMILES：生成唯一ID+特征映射
    4. 为副本添加SMILES ID列
    5. 处理log10_km列（复用/新增，适配Km任务）
    6. 删除指定冗余列
    7. 保存处理后的pkl副本（保留ezy_feat原始格式）
    """
    # -------------------------- 步骤1：加载原始Km数据（只读，不做任何修改） --------------------------
    print(f"===== 开始加载原始Km数据集（只读） =====")
    if not os.path.exists(RAW_DATA_PATH):
        raise FileNotFoundError(f"原始Km数据集不存在：{RAW_DATA_PATH}")
    ...
    print(f"✅ 原始Km数据加载完成，ezy_feat保持原始格式")
    ...
    if os.path.exists(SMILES_ID_FEAT_PATH):
        ...
        print(f"检测到Km底物ID-特征映射文件：{SMILES_ID_FEAT_PATH}，直接加载...")
        ...
        # 不存在映射文件，重新生成
        print(f"未检测到Km底物ID-特征映射文件，开始生成...")
        # 提取副本中的唯一SMILES
        ...
        print(f"✅ Km底物ID-特征映射已保存到：{SMILES_ID_FEAT_PATH}")

    # -------------------------- 步骤4：为Km数据副本添加SMILES ID列 --------------------------
    df['smiles_id'] = df['Smiles'].map(smiles_to_id)
    # 验证SMILES ID是否添加成功（无缺失值）
    if df['smiles_id'].isnull().any():
        print(f"⚠️  Km数据集存在部分SMILES未映射到ID，缺失数量：{df['smiles_id'].isnull().sum()}")
    else:
        print(f"✅ 已为Km数据副本添加smiles_id列，无缺失值")

    # -------------------------- 步骤5：处理log10_km列（复用/新增，适配Km任务） --------------------------
    print(f"\n===== 开始处理log10_km列 =====")
    if 'log10_km' in df.columns:
        # 复用已有列，确保列名统一，不修改原始值
        df['log10_km'] = df['log10_km'].copy()
        print(f"✅ 已复用现有log10_km列，值保持不变")
    elif 'log10_KM' in df.columns:
        # 兼容大写列名，统一为log10_km
        df['log10_km'] = df['log10_KM'].copy()
        print(f"✅ 检测到log10_KM列，已复制并统一列名为log10_km")
    else:
        # 无现有列，基于Km相关列计算（适配Km数据集字段）
        print(f"未检测到log10_km列，基于Km数值列计算...")
        # 自动识别Km数值列（优先匹配常见字段名）
        km_value_col = None
        for col in ['Km(M)', 'km', 'KM']:
            if col in df.columns:
                km_value_col = col
                break
        if km_value_col is None:
            raise ValueError("Km数据集中未找到Km数值列（如Km(M)、km、KM），无法计算log10_km")

        def calc_log10_km(x):
            if pd.isna(x) or x <= 0:
                return -10000000000  # 异常值标记（与后续训练逻辑对齐）
            return math.log10(x)

        df['log10_km'] = df[km_value_col].apply(calc_log10_km)
        print(f"✅ 已基于{km_value_col}列新增log10_km列，异常值标记为：-10000000000")

    # -------------------------- 步骤6：删除指定冗余列 --------------------------
    print(f"\n===== 开始删除Km数据集指定冗余列 =====")
    # 要删除的列列表（可根据需求调整，兼容Km数据集可能的冗余列）
    cols_to_drop = ['sbt_feat', 'sequence_length', 'new_ezy_feat', 'seq_length', 'sbt_feat_temp']
    # 过滤掉不存在的列，避免删除报错
    ...
    print(f"处理后Km数据副本形状：{df.shape}")
    print(f"处理后Km数据副本列名：{df.columns.tolist()}")
    ...
    print(f"✅ Km数据集ezy_feat保持原始格式，未做任何修改")

    # -------------------------- 步骤7：保存处理后的Km pkl副本（保留ezy_feat原始格式） --------------------------
    print(f"\n===== 开始保存处理后的Km数据集 =====")
    # pkl格式保存，完整保留ezy_feat的numpy数组格式，无丢失/变形
    df.to_pickle(PROCESSED_DATA_PATH)
    print(f"✅ 处理后的Km数据集已保存为：{PROCESSED_DATA_PATH}")
    print(f"✅ 保存格式为pkl，完整保留ezy_feat原始格式")
    print(f"✅ 原始Km数据集{RAW_DATA_PATH}未被修改，保持完整")
    ...
    print(f"6. 核心文件：")
    print(f"   - Km底物ID-特征映射：{SMILES_ID_FEAT_PATH}")
    print(f"   - 处理后Km数据集（保留ezy_feat原始格式）：{PROCESSED_DATA_PATH}")
    print(f"   - 原始Km数据集（只读）：{RAW_DATA_PATH}")
    print(f"\n===== Km数据集所有处理完成 =====")
