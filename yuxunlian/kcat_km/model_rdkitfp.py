...
class ModelFC(nn.Module):
    def __init__(self):
        ...
        # 🔴 核心修改1：将底物降维层整合到模型内部
        self.sbt_compress = nn.Linear(2048, 256)  # 935维→256维
    ...
    def forward(self, reactions, protein, enzyme_mask=None):
        # 🔴 核心修改2：在模型内部完成底物降维（原外部的sbt_compress逻辑移到这里）
        reactions = self.sbt_compress(reactions)  # [batch_size, 256]
        reactions = reactions.unsqueeze(1)  # [batch_size, 1, 256]（适配交叉注意力输入）
