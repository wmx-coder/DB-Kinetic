...
from model_maccs import Model_Regression  # 注：如果是单独文件，需确保导入路径正确，这里已内置模型定义
...
class CVDatasets():
    def __init__(self, fpath, batch_size=32):
        ...
        data_df = pd.read_pickle(fpath)[["new_ezy_feat", "maccs_feat", "log10_Km", "fold"]].copy()
        ...
        self.sbt_feats = np.array([
            f for f in data_df["maccs_feat"]
            if isinstance(f, np.ndarray) and f.ndim == 1 and len(f) == 167
        ])

        self.labels = data_df["log10_Km"].values
...
# ------------------------- 主训练逻辑（修改：移除外部sbt_compress相关代码） -------------------------
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Kcat_km Training (With Deepcopy)",
                                     formatter_class=RawDescriptionHelpFormatter)
    parser.add_argument("-data_fpath", type=str,
                        default="/mnt/usb3/code/wm/data/km_data/km_with_complete_feats_with_molt_maccs.pkl",
                        help="Dataset path")
    ...
    parser.add_argument("-model_save_dir", type=str,
                        default="/mnt/usb/code/wm/catapro/yuxunlian_models/kcat/km/model_maccs_1",
                        help="Directory to save best models")
    parser.add_argument("-log_dir", type=str, default="logfile_model_maccs_1", help="Log directory")
    parser.add_argument("-results_dir", type=str, default="results_model_maccs_1", help="Results directory")
    ...
    for fold in range(10):
        ...
        # 🔴 优化器仅传入model.parameters()（已包含内部的sbt_compress参数）
        optimizer = th.optim.Adam(
            model.parameters(),  # 无需再添加sbt_compress.parameters()
        ...
        for epoch in range(args.epochs):
            ...
            if is_best:
                # 核心修改2：更新最佳参数时用深拷贝（仅保存model.state_dict()，已包含sbt_compress）
                best_model_state = copy.deepcopy(model.state_dict())
        ...
        # 加载最佳模型（仅加载model_state，已包含sbt_compress参数）
        model.load_state_dict(best_model_state)

        # 重新验证（无需传入sbt_compress）
    ...
    # final_pred_df.to_csv("final_pred_label_model3_1.csv")
    ...
    final_metrics.to_csv(os.path.join(args.results_dir, "final_results_model_molt_1.csv"), index=False,
                         float_format="%.4f")
