...
from model_molt import Model_Regression  # 注：如果是单独文件，需确保导入路径正确，这里已内置模型定义
...
class CVDatasets():
    def __init__(self, fpath, batch_size=32):
        ...
        data_df = pd.read_pickle(fpath)[["new_ezy_feat", "molt5_feat", "log10_Km", "fold"]].copy()
        ...
        self.sbt_feats = np.array([
            f for f in data_df["molt5_feat"]
            if isinstance(f, np.ndarray) and f.ndim == 1 and len(f) == 768
        ])
...
if __name__ == "__main__":
    ...
    parser.add_argument("-model_save_dir", type=str,
                        default="/mnt/usb/code/wm/catapro/yuxunlian_models/kcat/km/model_molt_1",
                        help="Directory to save best models")
    parser.add_argument("-log_dir", type=str, default="logfile_model_molt_1", help="Log directory")
    parser.add_argument("-results_dir", type=str, default="results_model_molt_1", help="Results directory")
